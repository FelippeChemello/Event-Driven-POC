...
(async () => {
    ...
    await channel.consume(queue, async (message) => {
        ...
        const { fileBuffer, attempt = 1 } = JSON.parse(message.content.toString())
        const filename = message.fields.routingKey

        if (filename.endsWith('.pdf')) {
            fs.writeFileSync(path.resolve(receiveDir, filename), Buffer.from(fileBuffer))
        ...
        console.log(`File ${filename} failed to save, retrying (attempt ${newAttempt})...`)
        ...
        channel.publish('FILE_UPLOAD_EXCHANGE', filename, Buffer.from(JSON.stringify(newMessage)), { headers: { 'x-delay': 10000 } })
    }, {
        noAck: false,
