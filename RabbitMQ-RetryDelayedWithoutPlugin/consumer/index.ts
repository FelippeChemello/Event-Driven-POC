...
(async () => {
    ...
    await channel.assertQueue(delayQueue, { durable: true, deadLetterExchange: taskExchange, messageTtl: 10000 }); // Message TTL set the delay time for the message
    ...
    await channel.consume(taskQueue, async (message) => {
        ...
        const passed20SecondsFromTaskDate = taskDate.getTime() + 20000 < new Date().getTime()

        if (!passed20SecondsFromTaskDate) {
            retry += 1

            console.log(`[${new Date().toLocaleString()}] Task ${task} failed, retrying (attempt ${retry})...`)

            if (retry > 3) {
                console.log(`[${new Date().toLocaleString()}] Task ${task} failed too many times, discarding...`)

                return;
            }

            channel.publish(delayExchange, "", Buffer.from(JSON.stringify({ task, retry })))
        }
